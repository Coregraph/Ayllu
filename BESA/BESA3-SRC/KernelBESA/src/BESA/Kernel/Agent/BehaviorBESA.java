...
public class BehaviorBESA implements Runnable {
     ...
     * Kills the behavior.
     */
    final public void kill() throws KernellAgentExceptionBESA {
        while (!queue.isEmpty()) {
            try {
                Thread.sleep(this.milliseconds);
            } catch (Exception e) {
                ReportBESA.error("Happened an error on kill: " + e.getMessage());
                throw new KernellAgentExceptionBESA("Happened an error on kill: " + e.getMessage());
            }
        }
        ...
        try {
            ...
            throw new KernellAgentExceptionBESA("Couldn't send the internal kill event: " + e.toString());
        }
    }
     ...
     */
    final private EventBESA timeoutEvent() throws KernellAgentExceptionBESA {
        ...
        try {
            ...
            throw new KernellAgentExceptionBESA("Couldn't create time-out event: " + e.toString());
        }
        return ev;
    }
    ...
    final public void run() {
        ...
        try {            
            ...
            try {
                throw new KernellAgentExceptionBESA("Couldn't close the behavior guard: " + e.toString());
            } catch (KernellAgentExceptionBESA ex) {
                ex.printStackTrace();
            }
        }
     ...
     */
    final private synchronized EventBESA receiveEvent() throws KernellAgentExceptionBESA {
        //--------------------------------------------------------------------//
        ...
        if (this.periodic) {
            if (this.type == BehaviorBESA.PERIODIC) {
                while (queue.isEmpty()) {
                    try {
                        ...
                        throw new KernellAgentExceptionBESA("Happened an error on behavior periodic: " + e.getMessage());
                    }
                }
                ...
                try {
                    ...
                    throw new KernellAgentExceptionBESA("Happened an error on behavior time-out: " + e.getMessage());
                }
                if (queue.isEmpty()) {
                ...
                try {
                    ...
                    throw new KernellAgentExceptionBESA("Couldn't create the event of type \"BehaviorBESA.KILL_EVENT\":" + e.getMessage());
                }
            }
        } else {
            //----------------------------------------------------------------//
            ...
            while (queue.isEmpty()) {                                           //Checks if there are events, for input on wait.            
                try {
                    ...
                    throw new KernellAgentExceptionBESA("Happened an error on wait time of the event queue: " + ie.toString());
                }
            }
