...
import recursos.DatosAgenteRecursos;
...
public class GuardaControlarJigsawASHYI extends GuardBESA {
    ...
    public void funcExecGuard(EventBESA ebesa) {
        ...
        try {
            ...
            Estado_JigsawASHYI theMessage = null;
            ...
            switch (miEstado.getEstadoEstartegia()) {
                    ...
                    for(int i = 0;i <gruposB.size();i++)
                    {
                        for(int j = 0;j <gruposB.get(i).size();j++)
                        {
                           int y = 0;
                            ...
                            miEstado.setTotalEstudiantes(miEstado.getTotalEstudiantes()+1);
                        }
                            //crear datos de usuarios SMA y RA de cada usuario
                        new CreadorUsuarios().crearUsuarios(gruposB.get(i));
                        }
                ...
                case Estado_JigsawASHYI.ASIGNAR_TEMAS:
                    ...
                    int numeroMinimoTemas = miEstado.getGruposBase().size();//El minimo de temas debes er igual al numero de estudiantes por grupo
                   ...
                   // HashMap<Tema, List<GrupoPreguntas>> gruposPreguntasPorTema = Simulador.simularGruposPreguntasPorTemas(temas, numeroMinimoGruposPreguntas, numeroMinimoPreguntasPorPersona);
                    ...
                    datosAsignarTema.setTemasAsignadr(temas);
              ...
              case Estado_JigsawASHYI.CREAR_DIRECTORIOS:      
                  //crear directorio
                  int opcion = 1;
                  DatosCrearRecursos datosR = new DatosCrearRecursos(opcion);
                  datosR.setNombreLog("Log_Profesor");
                  datosR.setNumGruposBase(miEstado.getNumeroGruposBase());
                  datosR.setNumGruposTema(miEstado.getNumeroGruposTema());
                  
                  Ayllu_Data_Message datosInst = new Ayllu_Data_Message(Ayllu_EventTypes.AUTH_REQUEST, handlerCMA, datosR);
                  EventBESA evI = new EventBESA(GuardaCrearDirectorio.class.getName(), datosInst);
                  handlerCMA.sendEvent(evI);  
                  System.out.println("AYPUY: Agente lógico principal creado");
                  break;    
             case Estado_JigsawASHYI.ESPERAR_REGISTRO:
                    System.out.println("Paso " + miEstado.getEstadoEstartegia()+ " Inicia 5");
                    System.out.println("Esperando registro de estudiantes");
                    System.out.println("Paso " + miEstado.getEstadoEstartegia()+ " terminado 5");
                    break;
                  
             case Estado_JigsawASHYI.DAR_INSTRUCCIONES:      
                  //Profesor envia instrucciones de trabajo para grupos tméticos
                  handlerProf = miEstado.getCommunityAgentsHandlersbyRole("Profesor").get(0);
                  DatosInstrucciones datosEnvia = new DatosInstrucciones("instrucciones");
                  datosInst = new Ayllu_Data_Message(Ayllu_EventTypes.QUEST_REPLY, handlerCMA, datosEnvia);
                  evI = new EventBESA(GuardaDarInstrucciones.class.getName(), datosInst);
...
//                    break;
                case Estado_JigsawASHYI.DISCUSION_TEMATICA:                    
                    ...
                    for (int i = 0; i < miEstado.getNumeroGruposTema(); i++) 
                    {
                        ...
                        Tema tema = estadoCT.getMisDatos().getTemasAsignar().get(i);
                ...
                case Estado_JigsawASHYI.GENERAR_INFORME_TEMA:
                    ...
                    for (int i = 0; i < miEstado.getNumeroGruposTema(); i++) 
                    {
                        DatosAgenteRecursos datosRecursos = new DatosAgenteRecursos(miEstado.getDirectorio()+"\\Tematico\\Tema_"+i);
                        EstadoAgenteRecursos estadoR = new EstadoAgenteRecursos(datosRecursos);
                        AgenteRecurso agRecursos = new AgenteRecurso("Recursos_CMA_GIT"+i,estadoR, 0.91);
                        agRecursos.start();
                        DatosGenerarInformeTema datosGenerarInformeTema = new DatosGenerarInformeTema(i);
                        datosGenerarInformeTema.setAliasRecurso(agRecursos.getAlias());
                ...
                case Estado_JigsawASHYI.GENERAR_INFORME_GRUPO:
                    ...
                    for (int i = 0; i <miEstado.getNumeroGruposBase(); i++) 
                    {
                        DatosAgenteRecursos datosRecursos = new DatosAgenteRecursos(miEstado.getDirectorio()+"\\Base\\Base_"+i);
                        EstadoAgenteRecursos estadoR = new EstadoAgenteRecursos(datosRecursos);
                        AgenteRecurso agRecursos = new AgenteRecurso("Recursos_CMA_GIG"+i,estadoR, 0.91);
                        agRecursos.start();
                        miEstado = ((Estado_JigsawASHYI) this.getAgent().getState());
                        DatosGenerarInformeGrupo datosGenerarInformeGrupo = new DatosGenerarInformeGrupo(i);
                        datosGenerarInformeGrupo.setGrupos(miEstado.getGruposBase().get(i));
                        datosGenerarInformeGrupo.setAliasr(agRecursos.getAlias());
